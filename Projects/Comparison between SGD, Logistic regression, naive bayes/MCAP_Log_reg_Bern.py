...
def get_vocab_bern(path, file_list, vocabulary, test=0):
...
def create_vectors(list_of_vectors, vocabulary_list, spam=0):
    ...
    for i in range(len(list_of_vectors)):
        ...
        if i != 0:
            ...
            bern_array = np.append(bern_array, temp, axis=0)
        else:
            bern_array = np.array(email_vector)
            bern_array = np.reshape(bern_array, (1, len(vocabulary_list) + 1))
    return bern_array
...
vocab, list_of_spam_word_lists = get_vocab_bern(spam_path, list_of_spam_file_names, vocab)
vocab, list_of_ham_word_lists = get_vocab_bern(ham_path, list_of_ham_file_names, vocab)
...
train_data = final_bern_representation[:int(0.7*len(final_bern_representation)), :]
valid_data = final_bern_representation[int(0.7*len(final_bern_representation)):, :]
...
_, list_of_test_spam_word = get_vocab_bern(spam_test_path, list_of_spam_test_file_names, vocab, test=1)

# Create list of ham words for testing
_, list_of_test_ham_word = get_vocab_bern(ham_test_path, list_of_ham_test_file_names, vocab, test=1)
...
X_final = final_bern_representation[:, :-1].T
y_final = final_bern_representation[:, -1].T
